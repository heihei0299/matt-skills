...
<!-- 本区块由 matt-skills 管理；sync 会覆盖其中内容。项目专用规则请写在标记外或更具体路径的 AGENTS.md 中。 -->

## Scope

本区块定义受控本机开发环境中的跨项目通用项目级工作流。
项目专用的技术栈、命令、目录、测试方式和完成标准由标记外规则或更具体路径下的 `AGENTS.md` 定义；这些规则可补充或覆盖一般工作流，但不得隐式覆盖安全授权边界。
...
* 先建立项目支持的最小测试或可重复复现，再做最小实现；项目没有适用的自动化测试入口时，使用能够证明行为正确的最小可验证方式。
* 优先复用现有抽象、接口和依赖方向。
* 不创建平行实现，不扩大任务范围。
...
* 不读取或提交 secrets。
* 依赖安装或升级、部署、发布、`git push`、远程系统写操作及破坏性命令，须获得针对当前操作的用户明确授权。

## Completion

完成时说明改动或审查结论、已执行验证、未执行验证及剩余风险；如有 commit，再报告 commit hash。
