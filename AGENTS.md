...
3. 探索代码库：直接使用 `codegraph explore`（`codegraph CLI`）；若无 `.codegraph/` 索引先执行 `codegraph init` 初始化，再 `explore`
...
- 修改/实现 → 简单低风险直接执行：理解现状 → 最小修改 → 相关验证；有实际改动且验证通过时按一个用户请求执行一次 `git commit`；已确认 spec/ticket 的完整 test-first 交付 → `tdd-implement`；临时测试先行、TDD 或集成测试 → `tdd`；bug、失败、异常或性能问题 → `diagnose-fix`；其它中大型修改 → 先澄清范围、验收和验证方式，再按项目流程执行
...
理解/定位代码**必须**使用 `codegraph CLI`，直接优于 grep/find/读文件——一次调用拿到相关符号逐字源码与调用路径：
