...
// Seam 2: matt-skills update 提示已合并到 sync 且 exit 2
test('`update` 提示已合并到 sync 且 exit 2', () => {
  const { status, stdout, stderr } = runCli(['update']);
  assert.equal(status, 2, `expected exit 2, got ${status} stdout:${stdout} stderr:${stderr}`);
